...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.geometry("400300")
        self.title('Chat_lohiTalk')
        self.username = 'shiska nekit'
        ...
        self.open_img_button = CTkButton(self, text="P", width=50, height=40, command=self.open_image)
        self.open_img_button.place(x=0, y=0)

        self.adaptive_ui()

        self.add_message("Демонстрація відображення зображення:",
                         CTkImage(Image.open('Screenshot1.png'), size=(300, 300)))

        try:
            self.sock = socket(AF_INET, SOCK_STREAM)
            self.sock.connect(('localhost', 8080))
            hello = f"TEXT@{self.username}@[SYSTEM]{self.usernsme} приєднався до чату!\n"
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.label= CTkLabel(self.menu_frame, text="Imya")
            self.label.pack(pady=30)
            self.entry = CTkEntry(self.menu_frame, placeholder_text="type tvoy name blin")
    ...
    def save_name(self):
        ...
        if new_name:
            self.usernsme = new_name
            self.add_message(f'Tvoy new dickname: {self.username}')
